...
from loss import dice_bce_loss,dice_shape_loss,consistency_loss
...
def Net_Train():
    NAME = 'AL_NET' + Constants.ROOT.split('/')[-1]
    writer = SummaryWriter(Constants.log_dir+'AL_NET/')
    solver = MyFrame(AL_NET,consistency_loss, 1e-4)
    batchsize = torch.cuda.device_count() * Constants.BATCHSIZE_PER_CARD
    dataset = ImageFolder(root_path=Constants.ROOT, datasets='Cell',mode='train')
    ...
    # solver.loadtar('AL-Net/TargetA_weights/ResUNet34clusteredCell')
    ...
    for epoch in range(1, total_epoch + 1):
        ...
        for img, mask in data_loader_iter:
            ...
            train_loss,pred,maskt= solver.optimize()#_refalnetatloss,  _alnet
        ...
        solver.savetar(Constants.weight_dir + NAME , loadepoch, train_epoch_loss) 
        ...
        if train_epoch_loss >= train_epoch_best_loss:
            ...
            solver.save(Constants.weight_dir + NAME + '.th')
        if no_optim > Constants.NUM_EARLY_STOP:
            print('early stop at %d epoch' % epoch)
            solver.loadtar(Constants.weight_dir  + NAME)
            # break
